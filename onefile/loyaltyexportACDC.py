...
# NOWY układ Operations (Excel, wiersz nagłówków = 3)
COLS_O = {
    "pmid": "PMID",                         # gotowe PMID w pliku Operations
    "card": "ALL card number",              # nie jest kluczem, ale zostawiamy do ewentualnych analiz
    "holder": "Last name",
    "rev_hotel": "Revenue Hotel currency",
    "points1": "Rewards Points",            # w niektórych plikach:
    "points2": "Reward points",             # ...nazwa bywa bez 's'
    "credit": "Credit type",
    "dep": "Check-out date",                # data wyjazdu (opcjonalnie)
}
...
def znajdz_plik_operations(folder: Path) -> Path:
    # Teraz dostajemy Excela (xls/xlsx)
...
# =======================
# Czytanie Operations (Excel)
# =======================

def _normalize_ops(df: pd.DataFrame) -> pd.DataFrame:
    c = COLS_O
    # 1) Filtrujemy tylko "Hotel Stay"
    if c["credit"] not in df.columns:
        raise ValueError(f"Brak kolumny w Operations: '{c['credit']}'")
    mask_hotel = df[c["credit"]].astype(str).str.strip().str.upper() == "HOTEL STAY".upper()
    df = df[mask_hotel].copy()

    # 2) PMID – bierzemy bezpośrednio z kolumny i normalizujemy
    ...
    # 6) Punkty (opcjonalnie – jedna z nazw)
    points_col = c["points1"] if c["points1"] in df.columns else (c["points2"] if c["points2"] in df.columns else None)
    if points_col:
    ...
    # Dodatkowe podręczne pola
    if c["card"] in df.columns:
...
def wczytaj_operations(path: str) -> pd.DataFrame:
    ...
    try:
        df = pd.read_excel(path, dtype=str, header=2, engine=engine)  # nagłówki w 3. wierszu
...
def wczytaj_loyalty(path: str) -> pd.DataFrame:
    ...
    # Wyprowadzamy PMID z numeru karty
    df["karta_norm"]     = df[c["card"]].astype(str).apply(normalizuj_numer_karty)
...
def porownaj(lojal_df: pd.DataFrame, ops_df: pd.DataFrame, tolerancja: float = 0.10) -> Dict[str, pd.DataFrame]:
    ...
    freq = ops_tmp.groupby("nazwisko").agg(Wiersze=("nazwisko","size"),
                                           Wiersze_z_punktami=("ma_punkty","sum")).reset_index()
    ...
    wyniki = {
        ...
        "04_RÓŻNА_LICZBA_POZYCJI": pd.DataFrame(roznaliczb),
...
def safe_sheet_name(name: str, used: set) -> str:
    PREFER = {
        ...
        "04_RÓŻNА_LICЗБА_POZYCJI": "04_ROZNA_LICZBA",
